...
{
  // `outputs` must be captured by value to avoid dangling references.
...
{
  push_output(smt2_convt::convert_identifier(identifier_term.identifier()));
}
...
{
  auto id = function_application.function_identifier();
  auto arguments = function_application.arguments();
  push_outputs("(", std::move(id), std::move(arguments), ")");
}
...
public:
  ...
  void visit(const smt_define_function_commandt &define_function) override
  {
    ...
    join_strings(os, parameters.begin(), parameters.end(), " ");
